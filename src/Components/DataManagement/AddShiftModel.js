...
class AddShiftModel extends Component {
    constructor(props) {
        super(props);
        this.state = {
            addShiftModelShow: props.addShiftModelShow, loader: false, inTime: '', errorInTime: 0, outTime: '',
            errorOutTimee: 0, plantsCardList: [], clientPlanAreaDetailId: 0, pageLoader: true, shiftName: '', errorShiftName: false,
            MonColor: '#ffffff', TueColor: '#ffffff', WedColor: '#ffffff', ThuColor: '#ffffff',
            FriColor: '#ffffff', SatColor: '#ffffff', SunColor: '#ffffff', errroColor: false, startDateString: '',
            endDateString: '', CloneendDateString: '', ClonestartDateString: '', submitflag: 0,
            startTime: '', endTime: '', submitClick: 0, initTime: this.DisplayCurrentTime(),
            startTimershow: false, endTimershow: false, shiftError: false, shiftErrorMsg: "", timeError: false
        }
    }
    componentDidMount() {
    ...
    setModalHide = () => {
        this.props.addShiftModelHide(false);
    }
    selectPlant = (Object) => {
    ...
    startDateEvent = date => {
        ...
        // this.setState({startDateString:date});
        this.setState({ startDateString: date });
        this.setState({ ClonestartDateString: date });
    }
    endDateEvent = date => {
        // const Param = new Date(object); 
        // const DateValue = Param.getFullYear()+'-'+Param.getMonth()+'-'+Param.getDate();
        ...
        this.setState({ endDateString: date });
        this.setState({ CloneendDateString: date });
    }
    ymdConvert = (object) => {
        const Param = new Date(object);
        const DateValue = Param.getFullYear() + '-' + Param.getMonth() + '-' + Param.getDate();
    ...
    render() {
        return (
            <Modal id="rosterManagementModal-assign" className="addShiftModel" show={this.state.addShiftModelShow} onHide={this.setModalHide} size="sm"
                aria-labelledby="contained-modal-title-vcenter" centered>
                <Modal.Header closeButton>
                    <Modal.Title id="contained-modal-title-vcenter" className="pl-1">Add New Shift</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Row>
                        {
                        ...
                        <Col xl="12">
                            <Row className="align-items-center">
                                ...
                                <Col xs={9} sm="9" xl="8 pl-0" className="addPlantForm">
                                    <Form.Group>
                                        ...
                                        {
                                            this.state.clientPlanAreaDetailId == '0' && this.state.submitflag == 1 ? (
                                ...
                                <Col xs={9} sm="9" xl="8 pl-0" className="addPlantForm">
                                    <Form.Group>
                                        ...
                                        />
                                        {this.state.shiftName == '' && this.state.submitflag == 1 ? (
                            ...
                            <Col xs={9} sm="9" xl="8 pl-0" className="addPlantForm">
                                <Form.Group className="datePicker">
                                    <DatePicker id="startDate" selected={this.state.startDateString} placeholderText="Start Date"
                                    minDate={new Date()} dateFormat="dd-MM-yyyy" onChange={this.startDateEvent.bind(this)}/>
                                    <i className="calIcon"></i> 
                                    {this.state.startDateString == '' && this.state.submitflag == 1 ? (                                        
                                        <Form.Text className="error-msg"> Please select start date </Form.Text>
                                        ):null
                                    }                                 
                                </Form.Group>                               
                            </Col>
                            ...
                            <Col xs={9} sm="9" xl="8 pl-0" className="addPlantForm">
                                <Form.Group className="datePicker">
                                    <DatePicker  id="endDate"  selected={this.state.endDateString} placeholderText="End Date"
                                    minDate={new Date()} dateFormat="dd-MM-yyyy" onChange={this.endDateEvent.bind(this)}/>
                                    <i className="calIcon"></i>
                                    {this.state.endDateString == '' && this.state.submitflag == 1 ? (                                        
                                        <Form.Text className="error-msg"> Please select start date </Form.Text>
                                        ):null
                                    }                                        
                                </Form.Group>
